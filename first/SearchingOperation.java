...
public class SearchingOperation {
	...
	static void searchByFirstName(List<Order> orderList) {
		orderList.stream()
		.filter(p -> p.getCustomer().getFirstName().startsWith("S"))
				.forEach(p -> System.out.print(p));
	}

	static void searchByLastName(List<Order> orderList) {
		orderList.stream()
		.filter(p -> p.getCustomer().getLastName().startsWith("M"))
		.forEach(p -> System.out.print(p));
	}

	static void searchByOrderId(List<Order> orderList) {
		orderList.stream()
		.filter(p -> p.getId() == 1001)
		.forEach(p -> System.out.print(p));
	}

	static void searchByZipcode(List<Order> orderList) {
		orderList.stream()
		.filter(p -> p.getCustomer().getAddress().getZipcode() == 20770)
				.forEach(p -> System.out.print(p));
	}
